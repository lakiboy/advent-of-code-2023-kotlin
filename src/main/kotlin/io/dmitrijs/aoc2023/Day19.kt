...
class Day19(input: String) {
    ...
    fun puzzle2(): Long {
        ...
        fun splitRanges(expr: String, ranges: VariableRanges): Pair<VariableRanges, VariableRanges> {
            val (name, split) = expr
                .substringBefore(':')
                .split('<', '>')
        ...
        fun traverse(workflow: String, ranges: VariableRanges): Long = when (workflow) {
            "R" -> 0L
            "A" -> ranges.variations()
            else -> {
                var result = 0L
                ...
                for (index in 0..<expressions.lastIndex) {
                    ...
                    if (left.variations() > 0) {
                        result += traverse(expressions[index].substringAfter(':'), left)
                    }
                    currentRanges = right
                }

                result + traverse(expressions.last(), currentRanges)
            }
        }

        return traverse("in", List(4) { 1 to 4_000 })
    }
